...
2.  **Set API Keys**: Create a `.env` file in the `backend/` folder with your `GEMINI_API_KEY`.
3.  **Launch Backend**:
    ```bash
    cd backend
    python -m venv venv
...
- **AI Engine**: Leverages `google-generativeai` (Gemini SDK) for analysis and optimization.
